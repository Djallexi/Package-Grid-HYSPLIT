...
hysplit_dispersion <- function(lat = 49.263,
  ...
  # NOUVEAU : Expand paths to absolute
  run_dir <- normalizePath(path.expand(run_dir), mustWork = TRUE)
  met_dir <- normalizePath(path.expand(met_dir), mustWork = TRUE)
  ...
  met_dir_abs <- normalizePath(path.expand(met_dir), mustWork = TRUE)
  
  for (i in 1:length(met)) {
    cat(met_dir_abs, "/\n", met[i], "\n",   # ← Utiliser met_dir_abs
        file = paste0(run_dir, "/", "CONTROL"),
        sep = '', append = TRUE)
  }
  ...
  if (get_os() == "mac") {
    hycs_std <- system.file("osx/hycs_std", package = "splitr")
    cmd <- paste0("(cd ", run_dir, " && ", hycs_std, ")")
    cat("Executing HYSPLIT (Mac):\n", cmd, "\n")
    result <- system(cmd)
    cat("Exit code:", result, "\n")
  }
  
  if (get_os() == "unix") {
    hycs_std <- system.file("linux-amd64/hycs_std", package = "splitr")
    
    if (hycs_std == "" || !file.exists(hycs_std)) {
      stop("hycs_std not found at: ", hycs_std)
    }
    
    # Vérifier que les fichiers requis existent
    required_files <- c("CONTROL", "SETUP.CFG", "ASCDATA.CFG")
    for (f in required_files) {
      if (!file.exists(file.path(run_dir, f))) {
        stop("Required file missing: ", f, " in ", run_dir)
      }
    }
    
    # AJOUTER LE CHEMIN VERS libgfortran.so.3
    lib_dir <- path.expand("~/lib")
    
    # Construire la commande avec LD_LIBRARY_PATH
    cmd <- paste0(
      "(cd ", run_dir, 
    ...
    cat("Executing HYSPLIT (Linux):\n", cmd, "\n")
    result <- system(cmd)
    cat("HYSPLIT exit code:", result, "\n")
    
    if (result != 0) {
      stop("HYSPLIT failed with exit code: ", result, 
           ". Check MESSAGE file in ", run_dir)
    }
    
    # Vérifier que PARDUMP a été créé
    if (!file.exists(file.path(run_dir, "PARDUMP"))) {
      if (file.exists(file.path(run_dir, "MESSAGE"))) {
        msg <- readLines(file.path(run_dir, "MESSAGE"))
        cat("\n=== HYSPLIT MESSAGE ===\n")
        cat(msg, sep = "\n")
      }
      stop("HYSPLIT ran but did not create PARDUMP file")
    }
    
    cat("✓ PARDUMP created successfully\n")
  }
  
  if (get_os() == "win") {
    hycs_std <- system.file("win/hycs_std.exe", package = "splitr")
    cmd <- paste0("(cd \"", run_dir, "\" && \"", hycs_std, "\")")
    cat("Executing HYSPLIT (Windows):\n", cmd, "\n")
    result <- shell(cmd)
    cat("Exit code:", result, "\n")
  }
  ...
  if (get_os() == "mac") {
    ...
    if (parhplot_path == "" || !file.exists(parhplot_path)) {
      stop("parhplot not found in splitr package for Mac")
    }
    cmd <- paste0("(cd ", run_dir, " && ", parhplot_path, " -iPARDUMP -a1)")
    cat("Executing parhplot (Mac):", cmd, "\n")
    result <- system(cmd)
    if (result != 0) {
      stop("parhplot failed with exit code: ", result)
    }
  }
  
  if (get_os() == "unix") {
    parhplot <- system.file("linux-amd64/parhplot", package = "splitr")
    
    if (parhplot == "" || !file.exists(parhplot)) {
      stop("parhplot not found in splitr package")
    }
    
    if (!file.exists(file.path(run_dir, "PARDUMP"))) {
      stop("PARDUMP not found - HYSPLIT may have failed")
    }
    ...
    # Vérifier que des fichiers GIS ont été créés
    gis_files <- list.files(run_dir, pattern = "^GIS_part.*\\.txt$")
    if (length(gis_files) == 0) {
      stop("parhplot ran but created no GIS_part files in ", run_dir)
    }
    cat("✓ parhplot created", length(gis_files), "GIS files\n")
  }
  
  if (get_os() == "win") {
    parhplot_path <- system.file("win/parhplot.exe", package = "splitr")
    if (parhplot_path == "" || !file.exists(parhplot_path)) {
      stop("parhplot.exe not found in splitr package")
    }
    cmd <- paste0("(cd \"", run_dir, "\" && \"", parhplot_path, "\" -iPARDUMP -a1)")
    cat("Executing parhplot (Windows):", cmd, "\n")
    result <- shell(cmd)
    if (result != 0) {
      stop("parhplot failed with exit code: ", result)
    }
