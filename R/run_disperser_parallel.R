...
run_disperser_parallel <- function(input.refs = NULL,
  ...
  start_total <- Sys.time()        # début du chronomètre total
  times <- numeric(n)
  
  ## run the run_fac in parallel
  results <- parallel::mclapply(X = run_sample, mc.cores = mc.cores, FUN = function(i){
    
    start <- Sys.time()  # début chronométrage pour cette itération
    out <- run_fac(i, input.refs, pbl.height, species, npart, overwrite, keep.hysplit.files, proc_dir)
    end <- Sys.time()    # fin
    
    times[i] <<- as.numeric(difftime(end, start, units = "secs"))
    
    # estimation du temps restant
    elapsed <- sum(times[1:i])
    avg_time <- elapsed / i
    remaining <- avg_time * (n - i)
    message(sprintf("Iteration %d/%d done, est. remaining time: %.2f mins", i, n, remaining/60))
    
    return(out)
  })
  end_total <- Sys.time()
  message(sprintf("Total time for run_disperser_parallel: %.2f mins", difftime(end_total, start_total, units = "mins")))
